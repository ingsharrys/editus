    ...
    <div class="max-w-6xl mx-auto p-4">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 mb-6">
            <div class="flex flex-col items-center text-center gap-4">
                <img src="{{ $page->pictureUrl('square', 200, 200) }}" alt="Foto de {{ $page->name }}"
                    ...
                    <tbody>
                        @foreach ($posts as $post)
                            ...
                            <tr class="border-t align-top">
                                ...
                                <td class="py-2 px-2">
                                    @php
                                        // Preferimos el permalink o, si no, el link guardado
                                        $url = $post->fb_permalink_url ?: $post->link;

                                        // Si es video y tenemos fb_post_id:
                                        // - si NO hay url -> usar /reel/{id}
                                        // - si hay url pero NO es de Facebook/fb.watch -> forzar /reel/{id}
                                        if ($post->type === 'video' && $post->fb_post_id) {
                                            $isFacebook =
                                                $url &&
                                                (str_contains($url, 'facebook.com') || str_contains($url, 'fb.watch'));
                                            if (!$url || !$isFacebook) {
                                                $url = 'https://www.facebook.com/reel/' . $post->fb_post_id;
                                            }
                                        }
                                    @endphp
