...
{   /**
    ...
    {
        // 1) Token desde la pivote (tu caso principal)
        ...
        if ($pivot && !empty($pivot->page_access_token)) {
            ...
            Log::info('[FB] pivot token expirado; intentando refrescar vía /me/accounts', [
                'meta_page_id' => $metaPageId,
                'user_id'      => $userId,
            ]);
        }

        // 2) Necesitamos el page_id para buscar en /me/accounts
        ...
        // 3) User access token (solo columna access_token)
        ...
        // 4) Buscar token de página en /me/accounts y guardarlo en la pivote
    ...
    public function updatePostMetrics(MetaPost $post): bool
    {
        ...
        $changed = false;
        if (!is_null($alcance) && $alcance !== $post->alcance)                 { $post->alcance = $alcance; $changed = true; }
        if (!is_null($visualizaciones) && $visualizaciones !== $post->visualizaciones) { $post->visualizaciones = $visualizaciones; $changed = true; }
        if (!is_null($interacciones) && $interacciones !== $post->interacciones)       { $post->interacciones   = $interacciones; $changed = true; }
